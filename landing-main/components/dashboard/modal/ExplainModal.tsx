  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-[6px] border border-[#234D6C] bg-[#183B54]">
        ...
        <div className="text-xs text-gray-300 font-mono text-center sm:text-right border-t sm:border-t-0 sm:border-l border-[#234D6C] pt-2 sm:pt-0 sm:pl-4">
      ...
      <div className="space-y-2.5">
        ...
        <div className="space-y-2">
          {topContributors.map((c) => (
            <div
              key={c.num}
              className="flex items-center justify-between p-3 rounded-[4px] border border-[#234D6C] bg-[#183B54]/70 text-xs"
            >
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs font-bold text-[#E5B45A]">{c.num}.</span>
                  <strong className="text-white text-xs">{c.title}</strong>
                </div>
                <p className="text-[11px] text-gray-300 pl-4">{c.detail}</p>
              </div>
              <span className={`font-mono text-xs font-bold ${c.color} px-2 py-1 rounded bg-[#102F45] border border-[#234D6C]`}>
                {c.pts}
              </span>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
        <div className="p-3.5 rounded-[6px] border border-[#234D6C] bg-[#183B54] space-y-1.5">
          <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#D99018] block">
      ...
      <div className="space-y-2">
        ...
        <div className="divide-y divide-[#234D6C] rounded border border-[#234D6C] bg-[#183B54]">
          {evidenceSources.map((ev, idx) => (
            <div key={idx} className="p-2.5 flex items-center justify-between text-xs hover:bg-[#1A415E] transition-colors">
              <div className="flex items-center gap-2">
                <FileText className="h-3.5 w-3.5 text-[#D99018]" />
                <div>
                  <span className="font-bold text-white block text-xs">{ev.name}</span>
                  <span className="text-[10px] text-gray-400">{ev.ref}</span>
                </div>
              </div>
              <span className="text-[10px] font-mono text-[#2E8064] font-semibold bg-[#102F45] px-2 py-0.5 rounded border border-[#234D6C]">
                {ev.type}
              </span>
            </div>
          ))}
