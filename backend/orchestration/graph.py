...
class MPLADSOrchestrator:
    ...
    def node_run_all_agents(self, state: SystemState) -> SystemState:
        ...
        for agent_cls in ALL_SYSTEM_AGENTS:
            agent = agent_cls()
            try:
                ev = agent.run(context)
                evidence_list.append(ev)
            except Exception as e:
                # Agents use BaseAgent safety wrappers, but handle any edge case
                failed_ev = AgentEvidence.failed(
                    agent_id=getattr(agent_cls, "agent_id", "unknown_agent"),
                    agent_name=getattr(agent_cls, "agent_name", "Unknown Agent"),
                    error=str(e),
                )
    ...
    def node_investigation_routing(self, state: SystemState) -> SystemState:
        ...
        # Routing decision rule: overall risk >= 70.0 or RiskLevel in (HIGH, CRITICAL)
        is_high_risk = (
        ...
        if is_high_risk:
            # Construct CONTRACT 5 InvestigationIntake
            ...
            # Trigger Case Creation in Part A Investigation Engine
    ...
    def _build_graph(self):
        ...
        try:
            ...
            workflow.add_edge("fetch_digital_twin", "run_all_agents")
            workflow.add_edge("run_all_agents", "fuse_evidence")
            workflow.add_edge("fuse_evidence", "generate_explanation")
