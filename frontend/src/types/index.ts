...
// Keep File 2 legacy support for existing components
export type RiskTier = 'low' | 'medium' | 'high' | 'critical';
export interface MPLADSProject {
  id: string;
  code: string;
  title: string;
  category: 'Healthcare' | 'Education' | 'Sanitation' | 'Irrigation & Water' | 'Rural Roads' | 'Public Infrastructure' | string;
  state: string;
  constituency: string;
  ...
  status: 'Under Forensic Review' | 'Disbursal Frozen' | 'Show Cause Issued' | 'Field Team Dispatched' | 'Active Monitoring' | string;
  flaggedDate: string;
