...
export const mockRiskInferenceProvider = {
  async evaluateProject(input: ProjectAssessmentInput): Promise<RiskAssessmentResult> {
    // Artificial computation delay to simulate neural inference network pipeline
    ...
    const progressMismatchGap =
      Math.max(
        0,
        input.financialProgress -
    ...
    const bidDeviationPercentage =
      input.estimatedTenderAmount > 0
        ? Math.round(
            ((input.selectedBidAmount -
              input.estimatedTenderAmount) /
              input.estimatedTenderAmount) *
              100
          )
        : 0;
    ...
    const costOverrunPotential =
      input.estimatedProjectCost > 0
        ? Math.round(
            ((input.sanctionedAmount -
              input.estimatedProjectCost) /
              input.estimatedProjectCost) *
              100
          )
        : 0;
    ...
    if (utilizationRate > 85 && input.physicalProgress < 50) {
      financialScore += 35;
    }
    if (input.financialProgress > 80 && input.paymentCount > 4) {
      financialScore += 20;
    }
    if (costOverrunPotential > 20) {
      financialScore += 20;
    }
    financialScore = Math.min(100, Math.max(10, financialScore));

    let executionScore = 10;
    if (progressMismatchGap > 30) {
      executionScore += 45;
    } else if (progressMismatchGap > 15) {
      executionScore += 25;
    }
    if (timeOverrunDays > 60) {
      executionScore += 25;
    }
    if (input.currentStatus === 'Delayed') {
      executionScore += 15;
    }
    ...
    if (input.eligibleBidderCount === 1) {
      procurementScore += 35;
    } else if (input.eligibleBidderCount === 2) {
      procurementScore += 18;
    }
    if (bidDeviationPercentage > 25) {
      procurementScore += 30;
    } else if (bidDeviationPercentage > 10) {
      procurementScore += 15;
    }
    if (input.contractorConcentrationPercentage > 40) {
      procurementScore += 20;
    }
    if (input.tenderType === 'Nomination') {
      procurementScore += 30;
    }
    procurementScore = Math.min(100, Math.max(8, procurementScore));

    let complianceScore = 10;
    if (!input.technicalSanctionAvailable) {
      complianceScore += 40;
    }
    if (!input.administrativeApprovalAvailable) {
      complianceScore += 25;
    }
    if (!input.documentationComplete) {
      complianceScore += 20;
    }
    if (!input.geoLocationVerified) {
      complianceScore += 15;
    }
    ...
    if (input.nearbySimilarWork) {
      duplicationScore += 25;
    }
    if (input.repeatedContractor) {
      duplicationScore += 20;
    }
    ...
    // raw weighted score calculations
    const rawWeighted =
      executionScore * 0.3 +
      financialScore * 0.25 +
    ...
    // Why flagged contributors compilation
    ...
    if (progressMismatchGap > 15) {
      whyFlaggedContributors.push({
        ...
        riskContributionPoints:
          progressMismatchGap > 35 ? 24 : 16,
      });
    }
    ...
    if (bidDeviationPercentage > 15) {
      whyFlaggedContributors.push({
        id: 'WF-02',
        index: String(
          whyFlaggedContributors.length + 1
        ).padStart(2, '0'),
        ...
        observedValue: `₹${(
          input.selectedBidAmount / 100000
        ).toFixed(1)}L`,
        referenceLabel: 'Estimated Tender',
        referenceValue: `₹${(
          input.estimatedTenderAmount / 100000
        ).toFixed(1)}L`,
        riskContributionPoints:
          bidDeviationPercentage > 30 ? 18 : 12,
      });
    }

    if (
      input.eligibleBidderCount <= 1 ||
      ...
      whyFlaggedContributors.push({
        ...
        title:
          'LOW BIDDER PARTICIPATION / NOMINATION',
        summary:
          input.tenderType === 'Nomination'
        ...
        observedValue:
          `${input.eligibleBidderCount} bidder`,
    ...
    if (
      input.duplicateWorkSuspected ||
      input.nearbySimilarWork
    ) {
      whyFlaggedContributors.push({
        id: 'WF-04',
        index: String(
        ...
        title:
          'POTENTIAL SCHEME OVERLAP / DUPLICATION',
        summary:
    ...
    if (
      !input.technicalSanctionAvailable ||
      ...
      whyFlaggedContributors.push({
        ...
        title:
          'STATUTORY DOCUMENTATION DEFICIT',
        summary:
    ...
    if (whyFlaggedContributors.length === 0) {
      whyFlaggedContributors.push({
        ...
        referenceLabel:
          'Tender Competition',
        referenceValue:
          `${input.eligibleBidderCount} Bidders`,
    ...
    if (progressMismatchGap > 10) {
      riskFactors.push({
        ...
        category: 'EXECUTION',
        observedValue:
          `${input.financialProgress}% financial / ${input.physicalProgress}% physical`,
        referenceValue:
          '±10% allowable progress delta',
        deviation:
          `+${progressMismatchGap} percentage points gap`,
        riskContribution:
          progressMismatchGap > 30
            ? 24
            : 14,
    ...
    if (bidDeviationPercentage > 10) {
      riskFactors.push({
        id: 'RF-PROC-01',
        title:
          'Selected Bid Premium Over Estimated Cost',
        severity:
        ...
        observedValue:
          `+${bidDeviationPercentage}% over estimated tender`,
        referenceValue:
          'PWD Schedule of Rates tolerance (±5%)',
        deviation:
          `+${bidDeviationPercentage}% deviation`,
        riskContribution: 16,
        whyItMatters:
    ...
    if (input.eligibleBidderCount <= 1) {
      riskFactors.push({
        id: 'RF-PROC-02',
        title:
          'Single-Bidder / Low Competition Tender',
        severity: 'HIGH',
        category: 'PROCUREMENT',
        observedValue:
          `${input.eligibleBidderCount} eligible bidder`,
        referenceValue:
          'Minimum 3 competitive bids recommended',
        deviation:
          'Single source participation',
        riskContribution: 15,
        whyItMatters:
    ...
    if (
      input.contractorConcentrationPercentage > 35
    ) {
      riskFactors.push({
        id: 'RF-PROC-03',
        title:
          'Contractor Volume Concentration',
        severity:
          input.contractorConcentrationPercentage > 60
            ? 'CRITICAL'
            : 'HIGH',
        category: 'PROCUREMENT',
        observedValue:
          `${input.contractorConcentrationPercentage}% of sector works`,
        referenceValue:
          '< 25% recommended single-vendor cap',
        deviation:
          `+${input.contractorConcentrationPercentage - 25}% above norm`,
        riskContribution: 12,
        whyItMatters:
    ...
    if (
      ...
      riskFactors.push({
        id: 'RF-COMP-01',
        title:
          'Statutory Documentation & Pre-Check Deficit',
        severity: 'CRITICAL',
        category: 'COMPLIANCE',
        observedValue:
          input.technicalSanctionAvailable
            ? 'Missing documentation'
            : 'Missing Technical Sanction',
        referenceValue:
          'Mandatory Technical Sanction & Land NOC',
        deviation:
          'Non-compliant statutory prerequisite',
        riskContribution: 20,
        whyItMatters:
    ...
    if (input.duplicateWorkSuspected) {
      riskFactors.push({
        id: 'RF-DUP-01',
        title:
          'Suspected Cross-Scheme Asset Duplication',
        severity: 'CRITICAL',
        category: 'DUPLICATION',
        observedValue:
          'Potential overlap flagged with existing asset',
        referenceValue:
          'Distinct non-duplicated civil asset',
        deviation:
          'Spatial/asset duplication risk',
        riskContribution: 22,
        whyItMatters:
          'Double-charging civil works against both MPLADS and Central/State schemes represents a major audit para.',
        recommendedVerification:
    ...
    if (riskFactors.length === 0) {
      riskFactors.push({
        ...
        category: 'COMPLIANCE',
        observedValue:
          'All monitored metrics within statutory limits',
        referenceValue:
          'MPLADS Guideline baseline',
        deviation:
          'Nominal variance',
        riskContribution: 0,
        whyItMatters:
    ...
    // Compare matrix compilation
    const compareWithExpected: CompareMetricItem[] = [
      {
        label:
          'Financial vs Physical Progress',
        observed:
          `${input.financialProgress}% fin / ${input.physicalProgress}% phy`,
        expected: '±10% delta',
        difference:
          `${progressMismatchGap} pts gap`,
        status:
          progressMismatchGap > 30
            ? 'CRITICAL EXCEPTION'
...

      {
        ...
        expected: '±5% PWD SoR',
        difference:
          `${bidDeviationPercentage > 0 ? '+' : ''}${bidDeviationPercentage}%`,
        status:
          bidDeviationPercentage > 25
            ? 'SIGNIFICANT MISMATCH'
...

      {
        ...
        observed:
          `${input.eligibleBidderCount} bidder(s)`,
        expected:
          '≥ 3 eligible bidders',
        difference:
          `${
            input.eligibleBidderCount < 3
              ? '-' +
                (3 -
                  input.eligibleBidderCount)
              : 'Compliant'
          }`,
        status:
          input.eligibleBidderCount <= 1
...

      {
        ...
        observed:
          `${timeOverrunDays} days overrun`,
        expected: '0 days',
        difference:
          `+${timeOverrunDays} days`,
        status:
          timeOverrunDays > 60
            ? 'SIGNIFICANT MISMATCH'
            : timeOverrunDays > 20
