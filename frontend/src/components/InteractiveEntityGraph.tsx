...
import { Network, ShieldAlert, AlertTriangle, Building2, UserCheck, Search, Link2, ExternalLink } from 'lucide-react';
...
export const InteractiveEntityGraph: React.FC<InteractiveEntityGraphProps> = ({
  ...
  return (
    <div id="interactive-entity-graph" className="bg-[#F1F0EC] rounded-[24px] p-5 sm:p-6 border border-[#E5E3DC] space-y-4">
      {/* Header & Controls */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-[#0E0E0E] text-white flex items-center justify-center">
            <Network className="w-4 h-4" />
          </div>
          <div>
            <h3 className="text-base font-semibold text-[#0E0E0E]">Interactive Contractor Collusion Nexus</h3>
      ...
      <div className="relative w-full h-[360px] bg-white rounded-2xl border border-[#E5E3DC] overflow-hidden select-none">
        ...
        <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-xs p-2 rounded-xl border border-[#E5E3DC] text-[10px] text-[#6B6B6B] flex items-center gap-3 shadow-2xs">
          ...
          <div className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-[#0E0E0E]" />
      ...
      {selectedNode && (
        ...
        >
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-[#0E0E0E] text-white uppercase">
                {selectedNode.type}
              </span>
          ...
          <div className="flex items-center gap-2">
            {onTriggerSubpoena && (
              <button
                ...
                className="bg-[#9FE870] hover:bg-[#8ee05c] text-[#0E0E0E] text-xs font-semibold px-4 py-2 rounded-full cursor-pointer transition-all"
              >
                Issue Section 14 Subpoena
              </button>
            )}
          </div>
        </motion.div>
