      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-5 space-y-4">
            <span className="text-xs font-mono font-bold uppercase tracking-widest text-[#002449] bg-[#D99016]/10 px-3 py-1 rounded-full border border-[#D99016]/30">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#002449] tracking-tight uppercase leading-tight font-sans">
              ...
              <span className="text-[#002449]">Thousands of Works.</span>
            </h2>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <div className="p-3 rounded bg-[#FAFAF9] border border-[#E5E3DC]">
                <span className="text-[10px] font-mono text-[#6B6B6B] uppercase block">Total Monitored Outlay</span>
                <strong className="text-lg font-mono font-extrabold text-[#002449]">₹3,812 Cr</strong>
              </div>
              <div className="p-3 rounded bg-[#FAFAF9] border border-[#E5E3DC]">
                <span className="text-[10px] font-mono text-[#6B6B6B] uppercase block">Active Infrastructure</span>
                <strong className="text-lg font-mono font-extrabold text-[#002449]">7,842 Works</strong>
              </div>
            </div>

            <div className="pt-2">
              <Link to="/maps">
                <Button variant="default" size="sm" className="bg-[#002449] rounded-full text-white text-xs font-bold flex items-center gap-1.5 h-10 px-4 shadow-elevated">
          ...
          <div className="lg:col-span-7 rounded-[8px] border-2 border-[#002449] bg-[#FAFAF9] p-5 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-[#E5E3DC] pb-3">
              <div className="flex items-center gap-2">
                <Globe2 className="h-4 w-4 text-[#002449]" />
                <span className="text-xs font-bold uppercase tracking-wider text-[#002449]">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid grid-cols-4 w-full bg-white border border-[#E5E3DC]">
              ...
              <TabsContent value="states" className="space-y-2 pt-3">
                {statesData.map((reg) => (
                  <div
                    key={reg.state}
                    className="p-2.5 rounded bg-white border border-[#E5E3DC] flex items-center justify-between hover:border-[#002449] transition-colors"
                  >
                    <div>
              ...
              <TabsContent value="regions" className="space-y-2 pt-3">
                {regionsData.map((r) => (
                  <div key={r.region} className="p-2.5 rounded bg-white border border-[#E5E3DC] flex items-center justify-between">
                    <div>
                      <strong className="text-xs text-[#0E0E0E] block">{r.region}</strong>
                      <span className="text-[11px] text-[#6B6B6B] font-mono">{r.projects} • {r.value}</span>
                    </div>
                    <span className="text-xs font-mono font-bold text-[#002449] bg-[#FAFAF9] px-2 py-1 rounded border border-[#E5E3DC]">
                      {r.riskRate}
                    </span>
                  </div>
                ))}
              ...
              <TabsContent value="risks" className="space-y-2 pt-3">
                {riskData.map((rk) => (
                  <div key={rk.category} className="p-2.5 rounded bg-white border border-[#E5E3DC] flex items-center justify-between">
                    <div>
                      <strong className="text-xs text-[#0E0E0E] block">{rk.category}</strong>
              ...
              <TabsContent value="sectors" className="space-y-2 pt-3">
                {sectorsData.map((sec) => (
                  <div key={sec.name} className="p-2.5 rounded bg-white border border-[#E5E3DC] flex items-center justify-between">
                    <div>
                      <strong className="text-xs text-[#0E0E0E] block">{sec.name}</strong>
                      <span className="text-[11px] text-[#6B6B6B] font-mono">{sec.count} ({sec.value})</span>
                    </div>
                    <span className="text-xs font-mono font-bold text-[#002449]">
                      {sec.share} Share
                    </span>
                  </div>
                ))}
            ...
            <div className="pt-2 border-t border-[#E5E3DC] flex items-center justify-between text-[10px] text-[#6B6B6B] font-mono">
