    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-40 flex items-center gap-2 rounded-full bg-[#002449] text-white px-4 py-3 shadow-elevated border border-[#002449] hover:bg-[#002449] transition-all hover:scale-105"
        >
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-96 rounded-[8px] border-2 border-[#002449] bg-white shadow-2xl animate-in slide-in-from-bottom-5 duration-200">
          {/* Header */}
          ...
          {/* Quick Prompts */}
          <div className="p-3 bg-[#FAFAF9] border-b border-[#F1F0EC] space-y-1.5">
            ...
            <div className="flex flex-col gap-1">
              <button
                ...
                className="text-left text-[11px] text-[#002449] hover:text-[#002449] bg-white p-1.5 rounded border border-[#F1F0EC] hover:border-[#002449] truncate"
              >
          ...
          <div className="p-4 max-h-64 overflow-y-auto space-y-3">
            {response ? (
              <div className="space-y-2">
                <div className="p-3 rounded-[4px] bg-[#F7F5F0] border border-[#E5E3DC] text-xs text-[#1D2939] leading-relaxed">
                  {response}
                </div>
          ...
          {/* Search Form */}
          <form onSubmit={handleSubmit} className="p-3 border-t border-[#F1F0EC] bg-white flex gap-2">
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Query projects, rules, or anomalies..."
              className="flex-1 rounded-[4px] border border-[#E5E3DC] bg-white px-2.5 py-1.5 text-xs text-[#1D2939] focus:outline-none focus:border-[#002449]"
            />
